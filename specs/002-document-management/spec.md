...
1. **Given** ファイル名に「田中商事_請求書.pdf」「佐藤建設_契約書.docx」がある状態で、**When** 検索バーに「田中商事」と入力すると、**Then** 「田中商事_請求書.pdf」のみが表示され、ファイル名の「田中商事」がハイライトされる
...
事務スタッフが誤って「契約書」タイプで登録した文書を、詳細画面から「議事録」に変更する。ファイル名も「会議記録_20240115.pdf」に変更し、タグに「2024年度」を追加する。
