...
describe('PasswordField', () => {
  ...
  describe('基本的なレンダリング', () => {
    test('パスワードフィールドがレンダリングされること', () => {
      const r = render(<PasswordField value="" onChange={mockOnChange} />);
  ...
  describe('値の変更', () => {
    ...
    test('propsで渡された値が表示されること', () => {
      const r = render(
        <PasswordField value="initialValue" onChange={mockOnChange} />
      );
  ...
  describe('パスワード表示切り替え機能', () => {
    ...
    test('目のアイコンをクリックするとパスワードが表示されること', async () => {
      const user = userEvent.setup();
      const r = render(
        <PasswordField value="secret123" onChange={mockOnChange} />
      );
